...
import os
from modules.common import MCST


class GauntManyBody(torch.nn.Module):
    """TODO: implement multiple convolutions"""
    ...
    def forward(self, feature: torch.Tensor, weight=None, data=None, mask_edge=None, mask_sc=None):
        ...
        # size = [4*self.lmax+1, 4*self.lmax+1]

        # size = [2*self.v_max*self.lmax+1, 2*self.v_max*self.lmax+1]
        # image_fft = fft2(image, s=size)
        # B2_fft = image_fft[:, 0]*image_fft[:, 1]
        # Bvs_fft = [B2_fft]
        # B2 = ifft2(B2_fft, s=size)
        # for v in range(3, self.v_max+1):
        #     Bvs.append(Bvs[-1]*image_fft[:, v-1])
        # Bvs = torch.cat(Bvs, dim=1)
        # image = ifft2(Bvs, s=size)

        # # feature = torch.einsum("bcuv,iuv->bci", image[..., :2*self.lmax+1, :2*self.lmax+1], self.RCuvi_2l[:(self.lmax+1)**2]).real
        # start = (self.v_max-2)*self.lmax
        # end = self.v_max*self.lmax+1
        # feature = torch.einsum("bcuv,iuv->bci", image[..., start:end, start:end], self.RCuvi_2l[:(self.lmax+1)**2]).real
        # feature = self.postprcess(feature, weightc)

        Bvs = []
        pre_production = 1
        size = [2*self.v_max*self.lmax+1, 2*self.v_max*self.lmax+1]
        for v in range(2, self.v_max+1):
            cut0 = (v-2)*self.lmax
            cut1 = v*self.lmax+1
            if v == 2:
                pre_production = fft2(image[:, 0], s=size)*fft2(image[:, 1], s=size)
            elif v > 2:
                pre_production = pre_production*fft2(image[:, v-1], s=size)
            Bvs.append(ifft2(pre_production, s=size)[..., cut0:cut1, cut0:cut1])
        Bvs[0] = Bvs[0]*0
        Bvs = torch.cat(Bvs, dim=1)

        feature = torch.einsum("bcuv,iuv->bci", Bvs, self.RCuvi_2l[:(self.lmax+1)**2]).real
        feature = self.postprcess(feature, weightc)
...
class SelfInteraction(torch.nn.Module):
    def __init__(self,
        ...
        self.gmb = GauntManyBody(v_max=self.v_max, irreps_in=self.irreps_in, irreps_out=self.irreps_out)
        self.weight_gmb = e3nn.nn.FullyConnectedNet([num_type,
                                                    hidden_neurons,
                                                    self.gmb.weight_numel],
                                                    torch.nn.functional.silu)

    def forward(self, f_node, f_edge=None, node_emb=None, length_emb=None, data=None):
        weight = self.weight_gmb(node_emb)

        if self.split_stru > 1:
            sub_batch_size = math.ceil(data.num_nodes/self.split_stru)
            f_node = [[] for _ in range(self.split_stru)]
            for index_sub_batch in range(self.split_stru):
                ...
                sub_f_node = self.gmb(f_node[batch_slice], weight[batch_slice])
                f_node[index_sub_batch] = sub_f_node
            f_node = torch.cat(f_node, dim=0)
        else:
            f_node = self.gmb(f_node, weight)
...
if __name__ == "__main__":
    ...
    # not equivariant when v_max>2
    v_max = 4
    lmax = 4
    irreps_in = MCST(lmax, 1, 3)
    irreps_out = MCST(lmax, 1, 3)

    gmb = GauntManyBody(v_max, irreps_in, irreps_out)

    n = 2
    weight = torch.rand(size=(n, gmb.weight_numel))
    R_cpu = o3.rand_matrix(1).squeeze()
    D = gmb.irreps_out[0].D_from_matrix(R_cpu)
    input_random = irreps_in.randn(n, -1)
    input_random_rotated = torch.einsum("ij,zj->zi", D, input_random)

    f = gmb(input_random, weight)
    f_rotated = gmb(input_random_rotated, weight)
