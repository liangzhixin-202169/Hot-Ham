...
class GauntTensorProduct(torch.nn.Module):
    ...
    def forward(self, feature: torch.Tensor, sh: torch.Tensor, weight=None, data=None, mask_edge=None, mask_sc=None):
        ...
        if self.shared_weights:
            ...
            # weight3 = weight[..., self.w3_slice].reshape((-1,)+self.w3_shape)
            # weight3 = torch.cat([weight3[..., [l]].expand(-1, -1, 2*l+1) for l in range(self.lmax+1)], dim=-1)
            weightc = [weight[..., s].reshape((-1,)+self.wc_shape[i]) for i, s in enumerate(self.wc_slices)]

        image = torch.einsum("bci,bci,iuv->bcuv", feature.to(self.Ciuv.dtype), weight1.to(self.Ciuv.dtype), self.Ciuv)

        if len(sh.shape) == 2:
            weight2 = weight2.squeeze(1)
        kernel = torch.einsum("bi,bi,iuv->buv", sh.to(self.Ciuv.dtype), weight2.to(self.Ciuv.dtype), self.Ciuv)

        size = [4*self.lmax+1, 4*self.lmax+1]
        image_fft = fft2(image, s=size)
        kernel_fft = fft2(kernel, s=size)
        IK_fft = torch.einsum("bcuv,buv->bcuv", image_fft, kernel_fft)
        IK_ifft = ifft2(IK_fft, s=size)

        feature = torch.einsum("bcuv,iuv->bci", IK_ifft[..., :2*self.lmax+1, :2*self.lmax+1], self.RCuvi_2l[:(self.lmax+1)**2]).real
        ...
        for index, s in enumerate(self.channel_in_slice):
            ...
            # f = torch.einsum("bci,ca->bai", f, weight3[index])
...
class GauntTensorProduct_LCT(torch.nn.Module):
    """Gaunt tensor product (2D-FB) with local coordinate transformation, only used for convolution operations"""
    ...
    def forward(self, feature: torch.Tensor, sh: torch.Tensor, weight=None, wigner_D=None, mask_edge=None, mask_sc=None):
        ...
        # Reference: https://doi.org/10.1145/3478513.3480563
...
class GauntConvolution(torch.nn.Module):
    def __init__(self, irreps_node: o3.Irreps, irreps_out: o3.Irreps,  num_type: int, basis_size=12, sh_channel=None, hidden_neurons=64, split_stru=0, para=None, **kwargs):
        ...
        self.gtp = GauntTensorProduct_LCT(irreps_in1=irreps_in.copy(),
