...
from .common import MCST, Gate, E3LayerNormal
    ...
    def forward(self, x, sh, emb, wigner_D, mask_edge=None, mask_sc=None, weight=None):
        ...
        if self.edge_include_sc and self.shared_weights:
            ...
            for m in range(self.m_max+1):
                with torch.profiler.record_function("lin_m"):
...
class SO2_wrap(torch.nn.Module):
    ...
    def forward(self, x: torch.tensor, sh: torch.tensor, length_emb: torch.tensor, data: Data):
        with torch.profiler.record_function("so2conv"):
            weights = self.weights(length_emb)
            out = self.so2_lin(x=x, sh=sh, emb=length_emb, wigner_D=data.wigner_D, mask_edge=data.mask_edge, mask_sc=data.mask_sc, weight=weights)

        # sh_sc will be projected to 0e only: sh=Constant
        if self.edge_include_sc:
            if self.shared_weights:
                w = self.fc(length_emb[data.mask_sc])
                _x = self.sc_conv(x[data.mask_sc], x.new_tensor([[0.28209479177387814]]))
                out[data.mask_sc] += self.weight_multi(_x, w)
            else:
                w = self.fc(length_emb[data.mask_sc])
                out[data.mask_sc] += self.sc_conv(x[data.mask_sc], x.new_tensor([[0.28209479177387814]]), w)
        return out


class SO2OutputLayer(torch.nn.Module):
    def __init__(self, irreps_node: o3.Irreps, irreps_out: o3.Irreps,  num_type: int, basis_size=12, hidden_neurons=64, para=None):
        super().__init__()
    ...
    def forward(self, f_node, f_edge, sh, node_emb, length_emb, data: Data):
        ...
        with torch.profiler.record_function("SO2Conv"):
            f_edge = self.so2tp(f_cat, sh, length_emb, data)

        f_edge = self.gate(f_edge)

        f_node = scatter(f_edge, edge_dst, dim=0, dim_size=data.num_nodes, reduce="mean")
